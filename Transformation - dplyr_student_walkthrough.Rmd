...
```{r}
View(filter(flights, month == 3, day ==5))
...
```{r}
view(summarise(flights, delay = mean(dep_delay, na.rm = TRUE)))
...
by_destnation <- group_by(flights, dest)
delay <- summarise(by_destnation,
                   count = n(),
...
```{r}
# Which plane has the worst on_time record?
...
view(worst_ontime_record)

ggplot(data = worst_ontime_record , mapping =aes(x = count,y= avg_arr_delay ))+
         geom_point(alpha = 1/10)
...
```{r}
#what time of the day you shoud fly to avoid the  delays as much as possible?
flights_ny <- flights %>% filter(origin %in% c("JFK", "LGA", "EWR"))

# Filter out any rows with missing dep_time values
flights_ny <- flights_ny %>% filter(!is.na(dep_time))

flights_delay <- flights_ny %>% 
  mutate(hour = suppressWarnings(as.numeric(coalesce(format(dep_time, "%H", trim = TRUE), -1)))) %>% 
  filter(hour >= 0) %>% 
  group_by(hour) %>% 
